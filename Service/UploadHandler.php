...
use IonPot\AddressBook\Common\Util;
...
class UploadHandler
{
    ...
    public function handleUpload($uploadDirectory, $name = null)
    {
        ...
        if ($name === null) {
            ...
            $util = new Util();
            $imageName = pathinfo($name);
            $name = $util->cleanUrl($imageName["filename"]);
            $name = $name . "." . $imageName["extension"];
        }
    ...
    public function handleDelete($uploadDirectory, $name = null)
    {
        if ($this->isInaccessible($uploadDirectory)) {
            return array(
                'error' => "Server error. Uploads directory isn't writable" . ((! $this->isWindows()) ? " or executable." : ".")
            );
        }
    ...
    protected function toBytes($str)
    {
        ...
        switch ($last) {
            case 'g':
            case 'G':
                $val *= 1024;//intentional fall through
            case 'm':
            case 'M':
                $val *= 1024;//intentional fall through
            case 'k':
     ...
     *            The target directory to test access
     */
